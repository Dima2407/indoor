...
- (NSArray *)dijkstraSPTFrom:(id)source To:(id)target
{
    ...
    while (Q.count > [self visitedVertices].count) {
        ...
        if ([[dist valueForKey:u.label] integerValue] == INT_MAX) {
            break;
        }
        for (EKEdge *edge in u.adjacentEdges) {
            ...
            if ([alt integerValue] < [[dist valueForKey:v.label] integerValue]) {
    ...
    [smallest addObject:targetVertex];
    EKVertex *currVert, *prevVertex;
    currVert = targetVertex;
    int count = 0;
    
    while (prevVertex != sourceVertex) {
        count++;
        if (count > 2 * previous.count) {
            break;
        }
        
        if ([[previous valueForKey:currVert.label] isMemberOfClass:[NSNull class]]) {
            continue;
        }
        
        prevVertex = (EKVertex *)[previous valueForKey:currVert.label];
        [smallest addObject:prevVertex];
        currVert = prevVertex;
    }
    
    [self clearVisitHistory];
    NSArray* reversedArray = [[smallest reverseObjectEnumerator] allObjects];
    return reversedArray;
}

- (NSArray *)dijkstraShortestPathFrom:(CGPoint)sourcePoint To:(CGPoint)targetPoint {
    
    EKVertex *origin = [self getNearestVertexToPoint:sourcePoint];
    EKVertex *destination = [self getNearestVertexToPoint:targetPoint];
    
    return [self dijkstraSPTFrom:origin To:destination];
}

- (EKVertex *)getNearestVertexToPoint:(CGPoint)point {
    __block EKVertex *pathPoint = [EKVertex new];
    __block double proximity = 999999999999999;
    [_vertices enumerateObjectsUsingBlock:^(EKVertex *p, NSUInteger idx, BOOL *stop) {
        if ( sqrt(pow(p->x - point.x,2) + pow(p->y - point.y,2)) < proximity) {
            pathPoint = p;
            proximity = sqrt(pow(p->x - point.x,2) + pow(p->y - point.y,2));
        }
    }];
    ...
    for (EKVertex *vertex in Q) {
        if (!vertex.wasVisited) {
            ...
            if (!minDist) {
                ...
                if ([minDist integerValue] > [(NSNumber*)[dist valueForKey:label] integerValue]) {
                    minDist = [dist valueForKey:label];
                    index   = [Q indexOfObject:vertex];
                }
            }
