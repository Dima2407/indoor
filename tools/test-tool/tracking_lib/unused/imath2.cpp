...
namespace Math {
    ...
    std::vector<double> smooth(const std::vector<double> &measurements, const std::vector<long> &times, long max_time) {
        if (max_time == 0) {
            return measurements;
        }
    ...
    std::vector<double>  mean_filter(int n, const std::vector<double> &data, const std::vector<double> &t_weights) {
        if (data.size() < 2) {
            return data;
        }
